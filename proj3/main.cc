...
Gate *getGateWithX(Gate* g);//This function gets the first LOGIC_X value of the inputs of gate g
 ...
 */
bool podemRecursion(Circuit* myCircuit) {
//Check if there is a D or D_BAR at the POs
	vector<Gate*> poGates = myCircuit->getPOGates();
	for (int i=0; i<poGates.size(); i++) {
	char v = poGates[i]->getValue();
    	if ((v == LOGIC_D) || (v == LOGIC_DBAR)) {
	...
	char refFaultType = faultLocation->get_faultType();
//	calls the getObjective function and stores the result
	Gate * newFaultLoc = faultLocation;
	bool result = getObjective(newFaultLoc, refFaultType, myCircuit);	
	if (result == false) return false;
	Gate * PIgate;
	char PIValue;
	backtrace(PIgate, PIValue, newFaultLoc, refFaultType, myCircuit);

	if(faultLocation->getValue() == LOGIC_ONE)
		faultLocation->setValue(LOGIC_D);
	else if(faultLocation->getValue() == LOGIC_ZERO)
		faultLocation->setValue(LOGIC_DBAR);
	
	simFullCircuit(myCircuit);
	updateDFrontier(myCircuit);	
	
	if(faultLocation->getValue() == LOGIC_D)
		faultLocation->setValue(LOGIC_ONE);
	else if(faultLocation->getValue() == LOGIC_DBAR)
		faultLocation->setValue(LOGIC_ZERO);

	result = podemRecursion(myCircuit);
	if(result == true) return true;
	PIValue = LogicNot(PIValue);
...
bool getObjective(Gate* &g, char &v, Circuit* myCircuit) {
  ...
  // If you can successfully set an objective, return true.
	...
	if(g->getValue() != faultActivationVal){
		g = faultLocation;
...
void updateDFrontier(Circuit* myCircuit) {
  ...
  // like this.
	...
	int count = 0;
	for(int i = 0; i < myCircuit->getNumberGates(); i++){
		if(myCircuit->getGate(i)->getValue() != LOGIC_X){
			continue;
		}
		vector<Gate*> inputGates = myCircuit->getGate(i)->get_gateInputs();
		vector<Gate *>::iterator it = inputGates.begin();
		for(it; it != inputGates.end(); it++){
			if(((*it)->getValue() == LOGIC_D) || ((*it)->getValue() == LOGIC_DBAR)){
				dFrontier.push_back(myCircuit->getGate(i));
			}
		}	
	}
}
...
void backtrace(Gate* &pi, char &piVal, Gate* objGate, char objVal, Circuit* myCircuit) {
	...
	while(temp->get_gateType() != GATE_PI){
		switch(temp->get_gateType()){
			case GATE_NOT: case GATE_NOR: case GATE_NAND: case GATE_XNOR:{numInversions++; break;}
			default: numInversions += 0; 		
		}
		temp = getGateWithX(temp);
	}
	if(numInversions % 2 == 1)
		objVal = LogicNot(objVal);	
	pi = temp;
	piVal = objVal;
	pi->setValue(piVal);
}
...
char getNonControllingValue(Gate *g){
	char gateT = g->get_gateType();
	switch(gateT){
		case GATE_AND: case GATE_NAND:{	return LOGIC_ONE; }
		case GATE_OR: case GATE_NOR:{return LOGIC_ZERO;}
		case GATE_XOR: case GATE_XNOR:{return LOGIC_ZERO; } 
	}
}

//function returns the gate with gateValue X.
Gate* getGateWithX(Gate* g){
	vector<Gate*> inputGates = g->get_gateInputs();
	vector<Gate* >::iterator it = inputGates.begin();
	for(it; it != inputGates.end(); it++){
		if((*it)->getValue() == LOGIC_X){
			return (*it);
		}
	}	
}
