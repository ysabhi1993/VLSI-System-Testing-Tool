...
bool podemRecursion(Circuit* myCircuit) {
	...
	eventDrivenSimPart3(myCircuit, gateQueue1);
	...
	eventDrivenSimPart3(myCircuit, gateQueue2);
	result = podemRecursion(myCircuit);
	if(result) return true;
	...
	eventDrivenSimPart3(myCircuit, gateQueue3);
	return false;
}
...
bool getObjective(Gate* &g, char &v, Circuit* myCircuit) {
  ...
  // If you can successfully set an objective, return true.
	...
	//updateDFrontier(myCircuit); //Comment this line for part-3
